...
module.exports = {
  ...
  list: async ctx => {
    ...
    ctx.state.data = await DB.query("SELECT review.id as review_id, review.movie_id as movie_id, review.avatar as avatar, review.content as content, review.audio as audio, review.second as second, review.username as username, movies.title as title, movies.image as image FROM review LEFT JOIN movies ON review.movie_id = movies.id where review.movie_id = ? order by review.create_time Desc", [movie_id])
  },
  ...
  add: async ctx => {
    ...
    let second = ctx.request.body.second
    if (content != null) {
      await DB.query('INSERT INTO review(user, username, avatar, content, movie_id) VALUES (?, ?, ?, ?, ?)', [user, username, avatar, content, movie_id])
    }
    if (audio != null) {
      await DB.query('INSERT INTO review(user, username, avatar, audio, second, movie_id) VALUES (?, ?, ?, ?, ?, ?)', [user, username, avatar, audio, second, movie_id])
    }
    ctx.state.data = {}
  }
}